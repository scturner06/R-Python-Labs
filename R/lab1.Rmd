...
The `rm()` function can be used to delete any that we donâ???Tt want:
...
Itâ???Ts also possible to remove all objects at once (effectively clearing the history):
...
that is given in the functionâ???Ts help file. As this example illustrates, by
